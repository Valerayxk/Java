...
private class IntegralCalculator implements Callable<Double> {
    ...
    public Double call() {
        ...
        while (x + step <= upperLimit) {
            sum += step / 2 * 
                  (Math.cos(x * x) + Math.cos((x + step) * (x + step)));
            x += step;
        }
        sum += (upperLimit - x) / 2 * 
              (Math.cos(x * x) + Math.cos(upperLimit * upperLimit));
        return sum;
    }
}
   ...
   DefaultTableModel model = (DefaultTableModel) jTable1.getModel();
    ...
    for (int row = 0; row < model.getRowCount(); row++) {
        ...
        double range = (upperLimit - lowerLimit) / numThreads;
        double totalResult = 0.0;

        Future<Double>[] futures = new Future[numThreads];
        IntegralCalculator[] calculators = new IntegralCalculator[numThreads];

        for (int j = 0; j < numThreads; j++) {
            double threadLowerLimit = lowerLimit + j * range;
            double threadUpperLimit = threadLowerLimit + range;
            calculators[j] = new IntegralCalculator(threadLowerLimit, threadUpperLimit, step);
            futures[j] = executor.submit(calculators[j]);
        }

        for (int j = 0; j < numThreads; j++) {
            try {
                totalResult += futures[j].get();
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Ошибка при вычислении интеграла!", "Ошибка", JOptionPane.ERROR_MESSAGE);
            }
        }
